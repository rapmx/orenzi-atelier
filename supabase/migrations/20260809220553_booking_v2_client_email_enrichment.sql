-- HOTFIX Booking V2.2: enriquecimento de e-mail ausente no booking publico.
--
-- Causa raiz: find_or_create_client casa por telefone normalizado e, no ramo
-- "cliente ja existe", so faz coalesce de birthdate/instagram -- o e-mail NUNCA
-- e tocado. Cliente antiga com email NULL + booking novo informando e-mail
-- valido => email continua NULL => trg_notify_new_appointment envia
-- client_row.email = NULL para a Edge Function, que responde
-- {"skipped":"no client email"} e a confirmacao prometida nunca sai.
--
-- Por que NAO alterei find_or_create_client: ela tem 4 call sites. Alem desta
-- RPC, app/agendar.html:595 (booking V1 publico) tambem passa e-mail real --
-- mudar a funcao mudaria o comportamento do V1, fora do escopo deste hotfix.
-- Correcao minima e local: fica dentro de create_public_booking.
--
-- Regra implementada (booking publico enriquece dado AUSENTE, nunca edita CRM):
--   cliente novo                      -> find_or_create_client ja gravou; no-op
--   existente, email NULL/vazio       -> preenche
--   existente, mesmo email            -> no-op, permanece
--   existente, email DIFERENTE        -> NAO sobrescreve
...
BEGIN
  ...
  -- ENRIQUECIMENTO DE E-MAIL AUSENTE (hotfix). So preenche quando NAO ha
  -- e-mail. Cliente nova ja sai da find_or_create_client com e-mail
  -- preenchido (no-op); cliente existente com e-mail igual OU diferente tem
  -- email NOT NULL (no-op, nada e sobrescrito).
  ...
  RETURN jsonb_build_object(
    ...
    'staff', (SELECT jsonb_build_object('name', name, 'role', role) FROM public.staff WHERE id = v_staff_id),
